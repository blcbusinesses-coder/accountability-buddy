...
export default function TaskDetailScreen() {
  ...
  const GlassCardLocal = ({ children, style }: { children: React.ReactNode; style?: any }) => (
    ...
    <SceneBackground>
      <SafeAreaView style={[styles.container, { backgroundColor: 'transparent' }]} edges={['top']}>
        ...
        <View style={[styles.header, { borderBottomColor: theme.border }]}>
          ...
          <Text style={[styles.headerTitle, { color: theme.textPrimary, fontFamily: 'Outfit_600SemiBold' }]} numberOfLines={1}>Task Details</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: statusConfig.bg }]}>
            ...
            <Text style={[styles.statusText, { color: statusConfig.color, fontFamily: 'Outfit_600SemiBold' }]}>{statusConfig.label}</Text>
          </View>

          {/* Title + desc */}
          <Text style={[styles.taskTitle, { color: theme.textPrimary, fontFamily: 'Outfit_600SemiBold' }]}>{task.title}</Text>
          {task.description && (
            <Text style={[styles.taskDesc, { color: theme.textSecondary, fontFamily: 'Outfit_400Regular' }]}>{task.description}</Text>
          )}

          {/* Due date card */}
          <GlassCardLocal style={{ marginBottom: 16 }}>
            <View style={styles.infoRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.infoValue, { color: theme.textPrimary, fontFamily: 'Outfit_500Medium' }]}>
            ...
            <View style={styles.infoRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.infoValue, { color: isOverdue ? theme.warning : theme.textPrimary, fontFamily: 'Outfit_500Medium' }]}>
          ...
          {canSubmit && (
            ...
            >
              ...
              <Text style={[styles.submitBtnText, { fontFamily: 'Outfit_700Bold' }]}>Submit Proof of Completion</Text>
            </TouchableOpacity>
          )}
          ...
          {isOverdue && (
            <View style={[styles.overdueCard, { backgroundColor: theme.warningMuted, borderColor: theme.warning + '44' }]}>
              ...
              <Text style={[styles.overdueText, { color: theme.warning, fontFamily: 'Outfit_400Regular' }]}>This task is overdue and will be marked as failed.</Text>
            </View>
          )}

          {/* Submissions */}
          <Text style={[styles.sectionTitle, { color: theme.textSecondary, fontFamily: 'Outfit_600SemiBold' }]}>
            Submissions ({submissions.length})
          </Text>

          {submissions.length === 0 ? (
            <Text style={[styles.noSubs, { color: theme.textMuted, fontFamily: 'Outfit_400Regular' }]}>No submissions yet.</Text>
          ) : (
            submissions.map((sub) => (
              <GlassCardLocal key={sub.id} style={{ marginBottom: 10 }}>
                ...
                {sub.ai_reasoning && (
                  <Text style={[styles.reasoning, { color: theme.textSecondary, fontFamily: 'Outfit_400Regular' }]}>{sub.ai_reasoning}</Text>
                )}
                {sub.text_content && (
...
const styles = StyleSheet.create({
  ...
  taskTitle: { fontSize: 24, letterSpacing: -0.5, marginBottom: 8 },
  ...
  sectionTitle: { fontSize: 15, marginBottom: 12, marginTop: 4 },
